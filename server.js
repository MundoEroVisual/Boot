...
// Función para ejecutar un bot y mostrar logs completos
...
for (const envVar of REQUIRED_ENV) {
  if (!process.env[envVar]) {
    console.error(`❌ La variable de entorno ${envVar} no está definida en .env`);
  }
}
...
async function cargarNovelasAnunciadas() {
  try {
    ...
    console.warn(`[WARN] No se pudo cargar novelas anunciadas: ${e.message || e}`);
    return new Set();
  }
}
